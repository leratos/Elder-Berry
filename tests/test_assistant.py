...
class TestNoAction:
    def test_plain_response(self, assistant, mock_llm):
        mock_llm.generate.return_value = json.dumps(
            {"action": None, "params": {}, "response": "Hallo! Wie kann ich helfen?"}
        )
...
class TestErrorHandling:
    ...
    def test_missing_params(self, assistant, mock_llm):
        mock_llm.generate.return_value = json.dumps(
            {"action": "press_key", "params": {}, "response": "Taste gedrückt."}
        )
...
class TestAssistantResult:
    def test_result_fields(self):
        r = AssistantResult(
            response="ok", action_executed="press_key", action_success=True
        )
