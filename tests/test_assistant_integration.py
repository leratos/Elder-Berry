...
@pytest.fixture
def full_assistant(mock_llm, mock_db, mock_controller, mock_tts, character, mock_avatar):
...
class TestEmotionExtraction:
    ...
    def test_cleans_emotion_tag_from_response(self, full_assistant, mock_llm):
        mock_llm.generate.return_value = json.dumps({
            "action": None, "params": {},
            "response": "[angry] Das war nicht klug."
        })
    ...
    def test_no_emotion_tag_returns_neutral(self, full_assistant, mock_llm):
        mock_llm.generate.return_value = json.dumps({
            "action": None, "params": {},
            "response": "Einfach nur Text."
        })
        result = full_assistant.process("Test")
        assert result.emotion == "neutral"

    def test_emotion_none_without_character(self, mock_llm, mock_db, mock_controller, mock_tts):
        assistant = Assistant(
            llm=mock_llm, actions_db=mock_db,
            controller=mock_controller, tts=mock_tts,
        )
        mock_llm.generate.return_value = json.dumps({
            "action": None, "params": {},
...
class TestAvatarIntegration:
    ...
    def test_avatar_speaking_indicator(self, full_assistant, mock_llm, mock_avatar, mock_tts):
        mock_llm.generate.return_value = json.dumps({
            "action": None, "params": {},
            "response": "[neutral] Hallo"
        })
    ...
    def test_avatar_speaking_false_on_tts_error(self, full_assistant, mock_llm, mock_avatar, mock_tts):
...
class TestFullFlow:
    def test_action_with_emotion(self, full_assistant, mock_llm, mock_controller, mock_tts, mock_avatar):
        mock_llm.generate.return_value = json.dumps({
            "action": "press_key",
            "params": {"key": "enter"},
        ...
        mock_tts.speak.assert_called_once_with("Enter gedrückt! Los geht's!", emotion="motivated")
        mock_avatar.show_emotion.assert_called_once_with(Emotion.MOTIVATED)

    def test_result_has_emotion_field(self):
        r = AssistantResult(
            response="ok", action_executed=None,
            action_success=False, emotion="cheerful",
        )
